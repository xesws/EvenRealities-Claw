...
| 模块 | 职责 |
|---|---|
| `server.py` | aiohttp 服务：`/ws`（插件）、`/plugin/`（托管前端）、`/healthz`、`/admin/*`（仅 loopback）、离线会话 TTL 回收 |
...
```bash
PYTHONPATH=. .venv/bin/pytest tests/ -q          # 单元测试（208）
PYTHONPATH=. .venv/bin/python tests/e2e_sim.py   # 端到端闭环（22 项，自带 agent 夹具，无外部依赖）
```
