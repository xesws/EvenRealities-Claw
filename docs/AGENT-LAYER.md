...
| `remind_set` | WRITE | 200ms | **已实现** | 24 小时以内的提醒；到点由**网关**写屏 |
...
| `remind` | `remind_set` `remind_list` `remind_cancel` | 6000ms | **有** | **已实现** |

路由顺序即优先级：`remind` → `list` → `device` → `weather` → `math` → `daily` → `ask`。
`remind` 在 `list` 之前，两者的判据是**有没有「多久之后」**：
「提醒我买牛奶」是记一笔（`list`），「10 分钟后提醒我关火」是真定时（`remind`）。
