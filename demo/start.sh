...
#
#   ./demo/start.sh          替身模式：agent 换成 demo/fake_openclaw.py（本地没有 OpenClaw 时用）
#   ./demo/start.sh --real   真实模式：连本机真的 OpenClaw 网关（需它已在 18789 上跑着）
#
# 两种模式下，麦克风、faster-whisper 转写、HUD 状态机、折行分页、渲染节流
...
echo "${HEALTH}" | grep -q '"openclaw": true' || echo "⚠ healthz 显示 agent 侧未连上：${HEALTH}"
